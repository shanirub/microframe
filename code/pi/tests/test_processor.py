...
def test_deposit_success_opens_account(tmp_path: Path) -> None:
    p = _proc(tmp_path)
    out = p.handle(
        TxnRequest(tag=1, txn_type=TxnType.DEPOSIT, account_id="a", amount_cents=5_000)
    )
...
def test_balance_returns_current_balance(tmp_path: Path) -> None:
    p = _proc(tmp_path)
    p.handle(TxnRequest(tag=1, txn_type=TxnType.DEPOSIT, account_id="a", amount_cents=5_000))
    out = p.handle(TxnRequest(tag=2, txn_type=TxnType.BALANCE, account_id="a"))
    assert out.status is Status.SUCCESS
...
def test_withdraw_within_funds_applies(tmp_path: Path) -> None:
    ...
    p.handle(TxnRequest(tag=1, txn_type=TxnType.DEPOSIT, account_id="a", amount_cents=5_000))
    out = p.handle(
        TxnRequest(tag=2, txn_type=TxnType.WITHDRAW, account_id="a", amount_cents=2_000)
    )
    assert out.status is Status.SUCCESS
    # confirm it actually applied
    bal = p.handle(TxnRequest(tag=3, txn_type=TxnType.BALANCE, account_id="a"))
    assert bal.balance_cents == 3_000


def test_insufficient_funds_is_a_failed_result_not_an_exception(tmp_path: Path) -> None:
    p = _proc(tmp_path)
    p.handle(TxnRequest(tag=1, txn_type=TxnType.DEPOSIT, account_id="a", amount_cents=1_000))
    out = p.handle(
        TxnRequest(tag=2, txn_type=TxnType.WITHDRAW, account_id="a", amount_cents=5_000)
    )
...
def test_withdraw_unknown_account_is_not_found(tmp_path: Path) -> None:
    p = _proc(tmp_path)
    out = p.handle(
        TxnRequest(tag=7, txn_type=TxnType.WITHDRAW, account_id="ghost", amount_cents=100)
    )
    assert out.status is Status.FAILED
...
def test_balance_unknown_account_is_not_found(tmp_path: Path) -> None:
    p = _proc(tmp_path)
    out = p.handle(TxnRequest(tag=9, txn_type=TxnType.BALANCE, account_id="ghost"))
    assert out.status is Status.FAILED
    assert out.reason is Reason.NOT_FOUND
...
def test_malformed_negative_amount_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError):
        p.handle(
            TxnRequest(tag=2, txn_type=TxnType.WITHDRAW, account_id="a", amount_cents=-5)
        )
